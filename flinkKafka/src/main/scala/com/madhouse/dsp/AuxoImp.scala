...
object AuxoImp {
  def main(args: Array[String]): Unit = {
    ...
    val kafkaConsumer = new FlinkKafkaConsumer010(impTopic, BytesSchema, properties)

    val messageStream = env
      .addSource(kafkaConsumer)
      .map(r => {
        val t = AvroUtils.decode(r, ImpressionTrack.SCHEMA$).asInstanceOf[ImpressionTrack]
        val record = TrackerReport(IMP, timeprocess(t.getTime.toLong, 1800L), t.getProjectid.toInt,
    ...
    val jdbcConf = JDBCConf(mysqlUrl, mysqlUser, mysqlPasswd, impTable)
    println(s"#####jdbcConf = ${jdbcConf.toString}")

    messageStream.addSink(new MysqlSink(IMP, jdbcConf))

    env.execute("Kafka Flink Auxo: imp")
  }
}
